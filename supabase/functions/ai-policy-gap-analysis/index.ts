import { serve } from 'https://deno.land/std@0.168.0/http/server.ts';
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2.10.0';

const supabaseUrl = Deno.env.get('SUPABASE_URL') || '';
const supabaseServiceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') || '';
const openaiKey = Deno.env.get('OPENAI_API_KEY') || '';
const aiMode = Deno.env.get('AI_MODE') || 'mock';

const corsHeaders = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type',
};

serve(async (req) => {
  if (req.method === 'OPTIONS') {
    return new Response('ok', { headers: corsHeaders });
  }

  try {
    const {
      organizationId,
      useCaseId,
      title,
      description,
      controlsTally,
      evidenceTally,
      lang
    } = await req.json();

    const supabase = createClient(supabaseUrl, supabaseServiceKey, {
      auth: { persistSession: false }
    });

    const authHeader = req.headers.get('Authorization') || '';
    const token = authHeader.split(' ')[1];
    if (!token) {
      return new Response(JSON.stringify({ error: 'Missing Bearer Token' }), {
        status: 401,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }

    const { data: { user }, error: authErr } = await supabase.auth.getUser(token);
    if (authErr || !user) {
      return new Response(JSON.stringify({ error: 'Invalid user session JWT' }), {
        status: 401,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }

    if (aiMode === 'mock' || !openaiKey) {
      const mockRes = {
        task_type: 'policy_gap_analysis',
        organization_id: organizationId,
        use_case_id: useCaseId,
        risk_level: 'High',
        confidence: 0.88,
        rationale: 'Mock gap audit.',
        recommended_controls: ['CTL-012', 'CTL-013'],
        limitations: 'Simulated gaps mapping checks.',
        human_review_required: true,
        next_actions: ['Remediate evidence gaps.'],
        language: lang,
        generated_at: new Date().toISOString(),
        detected_gaps: ['Audit logs tracing missing.'],
        missing_controls: ['CTL-012', 'CTL-013'],
        policy_risks: ['EU AI Act Article 14 penalty risk.'],
        recommended_remediation: 'Establish continuos logs auditing.'
      };

      await supabase.from('llm_runs').insert({
        organization_id: organizationId,
        use_case_id: useCaseId || null,
        user_id: user.id,
        provider: 'Mock',
        model: 'Simulated Gap Engine',
        mode: 'mock',
        input_summary: JSON.stringify(req.body),
        output_summary: mockRes.rationale,
        structured_output: mockRes
      });

      return new Response(JSON.stringify(mockRes), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
    }

    // Query OpenAI
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${openaiKey}`
      },
      body: JSON.stringify({
        model: 'gpt-4o',
        messages: [
          {
            role: 'system',
            content: 'You are an AI GRC compliance analyst. Output policy gap analysis in JSON format.'
          },
          {
            role: 'user',
            content: `Use Case: ${title}\nDescription: ${description}\nControls: ${controlsTally}\nEvidence: ${evidenceTally}`
          }
        ],
        temperature: 0.4,
        response_format: { type: 'json_object' }
      })
    });

    const data = await response.json();
    const parsed = JSON.parse(data.choices[0].message.content);

    const finalRes = {
      ...parsed,
      task_type: 'policy_gap_analysis',
      organization_id: organizationId,
      use_case_id: useCaseId || null,
      generated_at: new Date().toISOString()
    };

    await supabase.from('llm_runs').insert({
      organization_id: organizationId,
      use_case_id: useCaseId || null,
      user_id: user.id,
      provider: 'OpenAI',
      model: 'gpt-4o',
      mode: 'live',
      input_summary: JSON.stringify({ title, controlsTally }),
      output_summary: finalRes.rationale || 'Policy gap analysis complete.',
      structured_output: finalRes
    });

    return new Response(JSON.stringify(finalRes), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    });
  } catch (err) {
    return new Response(JSON.stringify({ error: err.message }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    });
  }
});
